...
so the i need to extract all the transcations and convert into JSON and print the accurate results for all the transcations for given document
        ...
        Instruction:
               ...
               - For empty fields, must be "None" instead of "Null"
               - Some documents contains 2 or more pages of transactions, so for that continue with the transactions only, don't need to print user_id,name,bank, and all those things in the begenning for same costomer again
               - Strictly Must create a json for all the uploaded documents as per the instructions, don't skip anyone
{datas}
"""
